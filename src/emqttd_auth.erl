...
-import(emqttd_redis_client, [q/2, is_superuser/2]).
...
init({AuthCmd, SuperCmd, HashType}) ->
    io:format("auth init ~s~n",[AuthCmd]),
...
check(Client, Password, #state{auth_cmd  = AuthCmd,
                               ...
                               hash_type = HashType}) ->
     SuperCmd,
     %%io:format("auth request Password ~s,HashType ~s~n",[Password,HashType]),
     case q(AuthCmd, Client) of
                {ok, undefined} ->
                    {error, not_found};
                {ok, HashPass} ->
